...
import { ExecuteExternalApiTool, executeExternalApiSchema } from "./tools/execute-external-api-tool.js";
...
async function main() {
  try {
    ...
    const executeExternalTool = new ExecuteExternalApiTool();
    ...
    // Register execute-external-api tool
    server.tool(
      "execute-external-api",
      "Execute HTTP requests to external REST APIs (non-Amazon APIs)",
      executeExternalApiSchema.shape,
      async (params) => {
        const result = await executeExternalTool.execute(params);
        return {
          content: [
            {
              type: "text",
